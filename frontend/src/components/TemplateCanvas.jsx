function TemplateCanvas({ selectedSections, setSelectedSections }) {
  const moveUp = (index) => {
    if (index === 0) return;
  ...
  const toggleMode = (index) => {
    setSelectedSections((prev) =>
      prev.map((s, i) =>
        i === index
          ? { ...s, mode: s.mode === "combine" ? "separate" : "combine" }
          : s
      )
    );
  };
    ...
    <div className="builder">
      ...
      <div className="builder-canvas">
        {selectedSections.length === 0 ? (
          <div className="empty-state">Add sections to build template</div>
        ) : (
          selectedSections.map((section, index) => (
            <div key={index} className="template-block">
              ...
              <div className="template-actions">
                ...
                <button
                  className={`mode-pill mode-${section.mode || "combine"}`}
                  onClick={() => toggleMode(index)}
                  title="Toggle combined vs separate file"
                >
                  {section.mode === "separate" ? "Separate" : "Combine"}
                </button>
              </div>
            </div>
          ))
